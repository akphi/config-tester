...
const webpackConfig = webpackConfigCreate(undefined, { mode: 'development', enableAsyncTypeCheck: true });
// Type checking
...
tsCheckerHooks.issues.tap('fork-ts-checker-issues', (issues, compilation) => {
  ...
  issuesByFile.forEach((items, filePath) => {
    ...
    items.forEach(item => {
      const message = wrap(item.message, 72, { hard: true });
...
const server = new WebpackDevServer(compiler, { ...webpackDevServerConfig, hot: true });
